...
// #define MY_LEDS_NODE DT_PATH(myleds, led_ext)
...
int main(void)
{
	...
	err = bt_enable(NULL); // NULL -> no callback to notify completion -> enabling BT done synchronously
	if (err) {
	...
	printk("Initialization procedures complete\n");
